...
async def filter_work_message(message: Message, state: FSMContext):
    ...
    if str(user_support).isdigit():
        ...
        if len(get_user['user_login']) >= 1:
            await message.answer("<b>⛔ Бот находится на тех. работах.</b>",
...
async def filter_work_callback(call: CallbackQuery, state: FSMContext):
    ...
    await call.answer("⛔ Бот находится на тех. работах.", True)
...
async def main_start(message: Message, state: FSMContext):
    ...
    await message.answer("💳 Добро пожаловать в магазин <b>Sigma Logs</b>\n\n"
                        "Тут ты найдешь разнообразные логи с сортировкой по тематике заливов, частые пополнения товара и многое другое!\n\n"

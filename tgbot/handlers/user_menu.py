...
async def user_shop(message: Message, state: FSMContext):
    ...
    if len(get_all_categoriesx()) >= 1:
        await message.answer("<b>🎣 Выберите нужную тематику залива:</b>",
                            reply_markup=products_item_category_swipe_fp(0))
    else:
        await message.answer("<b>😔 Товар закончился.</b>")
...
async def user_support(message: Message, state: FSMContext):
    ...
    if str(user_support).isdigit():
        ...
        if len(get_user['user_login']) >= 1:
            await message.answer("☎ Поддержка работает с 9:00 до 22:00\n"
                                "👤 Саппорт №1: @golddacetup\n"
                                "👤 Саппорт №2: @scallytry\n\n"
...
async def user_purchase_category_next_page(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎣 Выберите нужную тематику залива:</b>",
...
async def user_purchase_category_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        ...
        await call.message.answer(f"<b>🎣 Текущая категория залива: <code>{get_category['category_name']}</code></b>",
                                reply_markup=products_item_position_swipe_fp(remover, category_id))
    else:
        if remover == "0":
            await call.message.edit_text("<b>😔 Товар закончился.</b>")
    ...
    if int(get_user['user_balance']) >= int(get_position['position_price']):
        if get_count == 1:
            ...
            await call.message.answer(ded(f"""
                                    ...
                                    ➖➖➖➖➖➖➖➖➖➖
                                    <b>🎦 ВКЛЮЧИТЕ ЗАПИСЬ ЭКРАНА ПЕРЕД ПОДТВЕРЖДЕНИЕМ!</b>
                                    """),
    ...
    if get_action == "yes":
        ...
        if 1 <= int(get_count) <= len(get_items):
            if int(get_user['user_balance']) >= amount_pay:
                ...
                await call.message.answer(ded(f"""
                                        <b>✅ Покупка совершена успешно!</b>
                                        
                                        ❗ <b>ВАЖНАЯ ИНФОРМАЦИЯ</b> ❗
                                        Не выключайте запись на время проверки лога!
                                        Также изучите комментарий от продавца в чеке.
                                        ➖➖➖➖➖➖➖➖➖➖
                                        ...
                                        Крайне советуем прочитать данную статью во избежание проблем с отработкой логов.
                                        <b>Ссылка на статью:</b> https://teletype.in/@golddace/6DemxKCXegS
                                        """),
